...
- **缺失 Bar 补充**：盘中断连缺失自动补零量 bar（`volume=0`），仅在同一交易时段内补充；首根 bar 到来时从开盘时间补起（开盘补bar）
...
首根 bar 到来时（无 last_bar），从交易时段开盘时间 → 首根 bar 之间补充：

- 补充 bar 的 OHLC = 首根 bar 的开盘价，volume = 0，turnover = 0
- 场景：9:45 才收到首根 bar，则补 9:30~9:44 共 15 根零量 bar
- 午盘同理：13:05 才收到首根 bar，补 13:00~13:04 共 5 根

> 两种补充 bar 统一走 `on_bar` 回调，通过 `volume=0` 区分。
...
- **缺失补充时段**：当前仅内置 A 股交易时段（9:30-11:30, 13:00-15:00），期货/期权夜盘需自行扩展 `TRADING_SESSIONS`
